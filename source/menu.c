...
#define APP_VERSION "DC-Switch Updater: 0.0.5"
...
void printOptionList(int cursor)
{
    ...
    char *option_list[]      = {    "Get Dcnigma's Pong Game?", \
                                    "Get Ebooks", \
                                    "Get Dcnigma's Amiibo dumps 3/11/2019", \
                                    "Update app", \
                                    "Reboot (reboot to payload)" };

    char *description_list[] = {    "Get Dcnigma's pong Example", \
                                    "Get Test File for Ebooks update", \
                                    "Get 179 Amiibo dumps made for Emuiibo", \
    ...
    for (int i=0, nl=0; i < (CURSOR_LIST_MAX+1); i++, nl+=NEWLINE)
    {
        ...
        else
        {
            ...
            // option text.
            drawText(fntSmall, 550, FIRST_LINE+nl, SDL_GetColour(dark_grey), option_list[i]);
            // description.
